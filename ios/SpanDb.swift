...
fileprivate let DB_FILE = "MiddlewareRum.sqlite"
...
class SpanDb {
    ...
    func store(spans: [OtelSpan]) -> Bool {
        if !ready() {
            return false
        }
        ...
        let maybeEncodedSpans = spans.map { ($0.startTime, try? encoder.encode($0)) }
    ...
    func truncate() -> Bool {
        ...
        if status != SQLITE_OK {
            print("span database vacuum failed: \(sqliteError(code: status))")
            return false
        }
    ...
    static func deleteAtDefaultLocation() {
        ...
        if FileManager.default.fileExists(atPath: path) {
            do {
                ...
                print("failed to delete span database at path \(path): \(error)")
            }
        }
    }
